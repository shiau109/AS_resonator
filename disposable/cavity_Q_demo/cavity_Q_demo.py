...
sample_name = "88v1"
project_folder = r"C:\Users\shiau\AS_resonator\disposable\cavity_Q_demo" # empty string "" for relative path 
attenuation = 110

# 1.1 File structure setting
raw_data_fd = f"{project_folder}\{sample_name}"
...
for cav_label, flist in subgroup_struc.items():
    ...
    powerQ_raws = []
    raw_dfs = []
    raw_demo = np.empty((2,501),dtype=complex)
    fit_demo = np.empty((2,501),dtype=complex)
    power_demo = np.empty(2)
    selected_idx = [10,5]
    for i, fn in enumerate(flist):
        ...
        raw_demo[i] = s21[selected_idx[i]]
        fit_demo[i] = fitCurves[selected_idx[i]]
        power_demo[i] = part_result["photons"][selected_idx[i]]
    ...
    plot_cavityS21_fitting( freq/1e9, raw_demo, fit_demo, np.array(["High Photon N","Low Photon N"]), title=None)# , output_fd=raw_data_fd )

    # plot_fitdata(raw_dfs)


    ## Save result
    save_power_dep(powerQ_result, f"{raw_data_fd}/{cav_label}.csv")

    # 2.2 Plotting
    # 2.2 plot setting
    plot_power_dependent_Q(powerQ_result, cav_label=cav_label) #, output_fd=raw_data_fd)
